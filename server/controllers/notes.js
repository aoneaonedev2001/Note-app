...
exports.createNote = (req, res) => {
  ...
  const sql =
    "INSERT INTO Note (title, content, created_at, updated_at, customer_id, category_id) VALUES (?, ?, NOW(), NOW(), ?, ?)";
  db.query(
    sql,
    [title, content, customer_id, category_id],
    (error, results) => {
      if (error) {
...
exports.updateNote = (req, res) => {
  ...
  const sql1 =
    "UPDATE Note SET title = ?, content = ?, category_id = ?, updated_at = NOW() WHERE note_id = ?";
  ...
  const sql2 =
    "INSERT INTO HistoryNote (note_id, action, edited_at) VALUES (?,  ?, NOW())";
